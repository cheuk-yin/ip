...
4. Run the bundled generator from the repository root. On Windows use the `python` launcher (this project's `python3` is a Microsoft Store stub); on macOS/Linux use `python3`:
...
`scripts/generate-split-view-diff.py` is the bundled standard-library-only generator (no dependencies to install). Keep the page self-contained except for optional syntax-highlighting resources loaded by the page from a CDN.
